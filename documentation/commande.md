...
| Investigation RER D (28 restants) : requêtes sur les Desserte isolées de la Ligne D (SQL brut) | **Pas un manque de topologie** : `troncons_rer.csv` contient déjà les arêtes de la branche Melun/Corbeil-Essonnes/Malesherbes. C'est une instance du problème déjà documenté "Stations dupliquées" (voir TODO.md) : les gares de cette branche existent en base sous une Station SANS `codeExterne` (utilisée par le RER D) distincte de leur jumelle AVEC `codeExterne` (utilisée par bus/Transilien R au même endroit), sans Correspondance entre les deux. Décision utilisateur : investiguer un correctif ciblé (pas la fusion complète des ~486 paires, déjà volontairement différée) — reste à faire. |
